...
def load_predictor(
    ...
    if _GLOBAL_PREDICTOR_STAGE1 is None:
        ...
        _GLOBAL_PREDICTOR_STAGE1 = LearnedDecompositionPredictor.from_pretrained(
            checkpoint_path_stage1, device=device
        )
        _GLOBAL_PREDICTOR_STAGE2 = LearnedDecompositionPredictor.from_pretrained(
            checkpoint_path_stage2, device=device
        )
...
def cache_step_learned(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    z2_forecast_method = cache_dic.get('z2_forecast_method', 'lagrange')
    ...
    with torch.no_grad():
        ...
        predicted = predictor.predict_from_decomposed(
            decomposed_cache_float, order=order, z2_forecast_method=z2_forecast_method
        )
...
def cache_step_merge_learned(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        last_pred = predictor.predict_from_decomposed(last_decomposed_float, order=last_order, z2_forecast_method=z2_forecast_method).to(original_dtype) if len(last_decomposed_float) > 0 else None
        curr_pred = predictor.predict_from_decomposed(curr_decomposed_float, order=curr_order, z2_forecast_method=z2_forecast_method).to(original_dtype)
