"""
Training script for qwen_edit LinCA two-stage invertible decomposition network

与 qwen_image 完全一致的:
- loss 设计 (compute_loss)
- interval 设计
- cond/uncond 都做训练
- 两阶段 Stage1=0-24, Stage2=25-49
- Early Stopping B
- wandb logging

差异:
- 数据: sample_XXXX, 22 val + 180 train + 11 display
- eval: 11 张展示图 (原图|生成图 concat)
"""
...
# 确保当前目录优先 (cache_functions 含 set_predictor_two_stage)，freqca_qwen 用于 pipeline
...
from dataset import MemoryEfficientCacheDataset, collate_fn, create_dataloader
from data_splits import build_splits, save_splits, load_splits
...
def train_epoch(
    ...
    """Run one two-stage training epoch（与 qwen_image 完全一致）"""
...
def _load_single_display_item(
    dataset_path: str,
    cache_data_dir: str,
    sample_idx: int,
) -> Dict:
    """从 cache metadata 和 dataset 加载单个展示样本"""
    idx_path = os.path.join(cache_data_dir, 'index.json')
    with open(idx_path, 'r') as f:
        idx_data = json.load(f)
    sample_info = None
    for s in idx_data['samples']:
        if s['sample_idx'] == sample_idx:
            sample_info = s
            break
    if sample_info is None:
        raise ValueError(f"sample_idx {sample_idx} not in index")

    meta_path = os.path.join(cache_data_dir, sample_info['dir'], 'metadata.json')
...
@torch.no_grad()
def eval_generate_images_edit(
    predictor_stage1: LearnedDecompositionPredictor,
    predictor_stage2: LearnedDecompositionPredictor,
    display_sample_indices: List[int],
    ...
    """生成 11 张展示图 (原图|生成图 concat)，与 qwen_image eval 对应"""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    if current_dir not in sys.path:
        sys.path.insert(0, current_dir)
    # 先导入 pipeline（会临时把 freqca_qwen 加入 path 并可能缓存其 cache_functions）
    from pipeline import QwenImageEditPipeline
    # 必须用本地的 cache_functions（含 set_predictor_two_stage）：pipeline 加载时会把 freqca 的 cache_functions 缓存在 sys.modules，此处清除后从当前目录重新导入
    to_remove = [k for k in sys.modules if k == "cache_functions" or k.startswith("cache_functions.")]
    for k in to_remove:
        del sys.modules[k]
    ...
    print(f"\n>>> Eval: generating {len(display_sample_indices)} display images (two-stage edit)...")
    ...
    for i, sample_idx in enumerate(display_sample_indices):
        item = _load_single_display_item(dataset_path, cache_data_dir, sample_idx)
        img = item['input_image'].resize((1024, 1024), Image.Resampling.LANCZOS)
        ...
        concat_w = orig_w + gen_w
        concat_h = max(orig_h, gen_h)
        concat_img = Image.new('RGB', (concat_w, concat_h), (255, 255, 255))
        concat_img.paste(img, (0, 0))
        concat_img.paste(gen_img, (orig_w, 0))

        output_path = os.path.join(display_output_dir, f'eval_{i:02d}_{item["task_type"]}.jpg')
        concat_img.save(output_path, quality=95)
        images.append(concat_img)
        print(f"  [{i+1}/{len(display_sample_indices)}] {item['task_type']}...")

        del result, generator, cache_dic, current, gen_img, concat_img
        if (i + 1) % 2 == 0:
            torch.cuda.empty_cache()
...
def main():
    parser = argparse.ArgumentParser(description="Train qwen_edit two-stage LinCA invertible decomposition network")
    ...
    if args.exp_name is None:
        args.exp_name = "qwen_edit_two_stage_v4"
    ...
    val_loader = create_dataloader(
        data_dir=args.cache_data_dir, batch_size=args.batch_size, intervals=intervals,
        num_workers=args.num_workers, shuffle=False, sample_indices=val_indices,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loader = create_dataloader(
            data_dir=args.cache_data_dir, batch_size=args.batch_size, intervals=intervals,
            num_workers=args.num_workers, shuffle=True, sample_indices=epoch_train_indices,
            subsample_seq=None, random_interval=args.random_interval,
        )
