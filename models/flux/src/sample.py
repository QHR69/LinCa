...
class SamplingOptions:
    ...
    learned_checkpoints: list[str]  # List of checkpoint paths, one per stage
    stage_splits: str           # Stage splits string, e.g. "12,12,13,13"
...
def main(opts: SamplingOptions):
    ...
    # Load learned predictor if using learned decomposition (多阶段)
    if opts.decompose_method == 'learned' and opts.learned_checkpoints and opts.stage_splits:
        from flux.modules.cache_functions.cache_utils_learned import load_predictor
        print(f"Loading multi-stage learned predictors (stage_splits={opts.stage_splits}):")
        for i, ckpt in enumerate(opts.learned_checkpoints):
            print(f"  Stage {i}: {ckpt}")
        load_predictor(opts.learned_checkpoints, opts.stage_splits, num_steps=opts.num_steps, device=str(device))
        print("Multi-stage learned predictors loaded successfully")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decompose_method', type=str, default='FFT', choices=['None', 'FFT', 'DCT', 'learned'])
    parser.add_argument('--use_z_cache', action='store_true')
    parser.add_argument('--forecast_steps', type=int, default=1)
    parser.add_argument('--learned_checkpoints', type=str, nargs='+', default=None,
                        help='Paths to learned predictor checkpoints, one per stage')
    parser.add_argument('--stage_splits', type=str, default='25,25',
                        help='Stage splits string, e.g. "12,12,13,13" for 4 stages')
    ...
    opts = SamplingOptions(
        ...
        learned_checkpoints=args.learned_checkpoints or [],
        stage_splits=args.stage_splits,
    )

    main(opts)
    # CUDA_VISIBLE_DEVICES=2 python src/sample.py --model_name flux-dev-kontext --add_sampling_metadata --interval 7 --use_z_cache --forecast_steps 7 --test_FLOPs
