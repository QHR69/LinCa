...
Hybrid design: lightweight Glow + RevNet residual, configurable partitioning, fixed prediction

Feature dimension: 3072
Prediction policy: fixed partitioning (0th/1st/2nd order), configurable partition sizes
...
class RevNetResidualBlock(nn.Module):
    ...
    F depends only on x2 and G only on y1, which makes the mapping exactly invertible
    """
...
class HybridInvertibleBlock(nn.Module):
    """
    Hybrid invertible block = lightweight 1x1 convolution + RevNet residual block (with dropout)
...
class InvertibleDecompositionNet(nn.Module):
    """
    Invertible decomposition network built from the hybrid architecture (with dropout)
    ...
    def decompose(self, x: torch.Tensor, split_dims: list = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Partitioning is configurable: split_dims gives the width of each partition
        """
...
class FixedPredictionStrategy(nn.Module):
    ...
    def predict_z1(
        ...
        """1st-order prediction: numerically guarded linear extrapolation (scalar and batched distances)"""
        eps = 1e-6

        # Batch tensor path: dt1/dt2 are [batch] tensors
        if isinstance(dt1, torch.Tensor) and dt1.dim() > 0:
            diff = (dt2 - dt1).abs()
            denom = diff.clamp(min=eps)

            # Reshape for broadcasting: [batch] -> [batch, 1, 1, ...]
            ndim_extra = z_prev.dim() - dt1.dim()
            dt1_b = dt1
            denom_b = denom
            diff_b = diff
            for _ in range(ndim_extra):
                dt1_b = dt1_b.unsqueeze(-1)
                denom_b = denom_b.unsqueeze(-1)
                diff_b = diff_b.unsqueeze(-1)

            slope = (z_prev - z_prev2) / denom_b
            slope = torch.clamp(slope, -1e4, 1e4)
            pred = z_prev + slope * dt1_b

            # Where diff < eps, fallback to z_prev (0-order)
            mask = (diff_b < eps)
            pred = torch.where(mask, z_prev, pred)

            # NaN/Inf per-element fallback
            bad = torch.isnan(pred) | torch.isinf(pred)
            pred = torch.where(bad, z_prev, pred)
            return pred

        # Scalar path (inference)
        if abs(dt2 - dt1) < eps:
            return z_prev.clone()
    ...
    def predict_z2(
        ...
        """2nd-order prediction: numerically guarded Lagrange interpolation (scalar and batched distances)"""
        eps = 1e-6
        ...
        if isinstance(dt1, torch.Tensor) and dt1.dim() > 0:
            ...
            for _ in range(ndim_extra):
                dt1_b = dt1_b.unsqueeze(-1)
                diff_12_b = diff_12_b.unsqueeze(-1)
                diff_23_b = diff_23_b.unsqueeze(-1)
                diff_13_b = diff_13_b.unsqueeze(-1)

            denom_12 = diff_12_b.clamp(min=eps)
            denom_23 = diff_23_b.clamp(min=eps)
            denom_13 = diff_13_b.clamp(min=eps)

            d1 = (z_prev - z_prev2) / denom_12
            d2 = (z_prev2 - z_prev3) / denom_23
            d1 = torch.clamp(d1, -1e4, 1e4)
            d2 = torch.clamp(d2, -1e4, 1e4)

            d2_diff = (d1 - d2) / denom_13
            d2_diff = torch.clamp(d2_diff, -1e4, 1e4)

            pred = z_prev + d1 * dt1_b + 0.5 * d2_diff * dt1_b * dt1_b

            # Where any diff is degenerate, fallback to z1 prediction
            degenerate = (diff_12 < eps) | (diff_23 < eps) | (diff_13 < eps)
            if degenerate.any():
                pred_z1 = self.predict_z1(z_prev, z_prev2, dt1, dt2)
                deg_b = degenerate
                for _ in range(ndim_extra):
                    deg_b = deg_b.unsqueeze(-1)
                pred = torch.where(deg_b, pred_z1, pred)

            # NaN/Inf per-element fallback
            bad = torch.isnan(pred) | torch.isinf(pred)
            if bad.any():
                pred_z1_fb = self.predict_z1(z_prev, z_prev2, dt1, dt2)
                pred = torch.where(bad, pred_z1_fb, pred)
...
class LearnedDecompositionPredictor(nn.Module):
    """
    Hybrid architecture + configurable partitioning + fixed prediction (gate-free, with dropout)
    ...
    def predict(
        ...
        Args:
            cache_list: cache entries [(dt1, feat1), (dt2, feat2), ...]
    ...
    def predict_from_decomposed(
        ...
        """
        Predict from an already decomposed cache (avoids recomputing forward)
    ...
    def precompute_inverse_weights(self):
        """Cache the inverse of every 1x1 convolution so inference skips torch.linalg.inv"""
    ...
    def from_pretrained(cls, checkpoint_path: str, device: str = 'cuda'):
        ...
        # Separate cached inverses: a None buffer is absent from the module state_dict
