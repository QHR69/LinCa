...
def add_inference_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        type=int,
        default=1,
        help="Stride for prompt sampling (1=all, 5=every 5th prompt).",
    )
...
def add_cache_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group(title="Cache acceleration args (flux_v4 method)")

    group.add_argument(
        "--interval",
        type=int,
        default=3,
        help="Cache interval: full computation every N steps.",
    )
    group.add_argument(
        "--max-order",
        type=int,
        default=2,
        help="Maximum prediction order for high-frequency components.",
    )
    group.add_argument(
        "--min-order",
        type=int,
        default=0,
        help="Minimum prediction order for low-frequency components.",
    )
    group.add_argument(
        "--first-enhance",
        type=int,
        default=3,
        help="Number of initial steps that always use full computation.",
    )
    group.add_argument(
        "--forecast-method",
        type=str,
        default="hermite",
        choices=["hermite", "taylor"],
        help="Forecasting method for cache prediction.",
    )
    group.add_argument(
        "--decompose-method",
        type=str,
        default="FFT",
        choices=["None", "FFT", "DCT", "learned"],
        help="Feature decomposition method.",
    )
    group.add_argument(
        "--use-z-cache",
        action="store_true",
        help="Enable two-period merging strategy.",
    )
    group.add_argument(
        "--forecast-steps",
        type=int,
        default=1,
        help="Number of cached steps per forecast period.",
    )
    group.add_argument(
        "--learned-checkpoint",
        type=str,
        default=None,
        help="Path to learned predictor checkpoint.",
    )
