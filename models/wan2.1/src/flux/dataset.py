...
class MemoryEfficientCacheDataset(Dataset):
    ...
    def _feature_path(self, prompt_dir: str, branch: str, step: int) -> str:
        """Support both Qwen nested files and Wan flat dual-stream files."""
        nested = os.path.join(self.data_dir, prompt_dir, branch, f'step_{step:02d}.pt')
        if os.path.exists(nested):
            return nested
        suffix = 'cond' if branch == 'cond' else 'uncond'
        return os.path.join(self.data_dir, prompt_dir, f'step_{step:02d}_{suffix}.pt')
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # Load the target feature
        target_path = self._feature_path(prompt_dir, branch, target_step)
        target_feature = torch.load(target_path, map_location='cpu', weights_only=True)
        # Cast to float32 (data is stored as bfloat16)
        target_feature = target_feature.float()
        ...
        step_distances = []
        for distance, step in cache_info:
            cache_path = self._feature_path(prompt_dir, branch, step)
            cache_feature = torch.load(cache_path, map_location='cpu', weights_only=True)
            # Cast to float32
            cache_feature = cache_feature.float()
